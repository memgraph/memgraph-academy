...
def init_data(card_count, pos_count):
    ...
    memgraph.execute("UNWIND range(0, {} - 1) AS id "
                     "CREATE (:Card {{id: id, compromised: false}})".format(
                         card_count))
    memgraph.execute("UNWIND range({}, {} - 1) AS id "
...
def pump_transactions(card_count, pos_count, tx_count, report_pct):
    ...
    for i in range(card_count + pos_count, card_count + pos_count + tx_count):
        memgraph.execute(query.format(rint(0, card_count),
                                      rint(card_count,
...
def get_graph():
    ...
    try:
        ...
        for result in results:
            ...
            if ((pos_id, transaction_id) not in links_set and
                    (transaction_id, pos_id) not in links_set):
        ...
        log.info(f"Data fetching went wrong: {e}")
        return ("", 500)
